...
int turnTo_c(setuser *person,int direct)//跳转函数
{
    if(direct==-1)//按情况判断
    {
        ...
        else if(strcmp(person->class,"cjwzs")!=0 && strcmp(person->class,"00000")!=0 && strcmp(person->class,"jmxzs")!=0)//权限码有误
        {
            return 1;
        }
    }
...
void changeGoods_c(setuser *head,setuser *person)
{
    ...
    for(ph=head->next;ph!=NULL;ph=ph->next)
    {
        if(strcmp(ph->accounts,person->accounts)==0 && strcmp(ph->code,ph->code)==0)//查找账户
        {
            strcpy(ph->score,person->score);
            strcpy(ph->goods,person->goods);
        }
    }
    if ((fp = fopen("data_c\\user\\userchange.txt", "wt")) == NULL)//以写的方式新建一个文件
    {
        closegraph();
        printf("Can't open userchange.txt");
    }
    for(ph=head->next;ph!=NULL;ph=ph->next)
    {
        fputc('@',fp);//@标志一个用户的开头
    ...
    fclose(fp);
    remove("data_c\\user\\userinf.txt");
    rename("data_c\\user\\userchange.txt","data_c\\user\\userinf.txt");
}